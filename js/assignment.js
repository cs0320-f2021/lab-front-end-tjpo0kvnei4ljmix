...
function setUpShop () {
    ...
    for (let i = 0; i < cart_buttons.length; i++){
        ...
        const item = document.querySelectorAll(item_id)
        btn.addEventListener("click", () => addToCart(item_id))
...
function displayCart () {
    ...
    const displayItem = function (item) {
        ...
        cart.innerHTML += `<div>Item ${item.id}, quantity: ${quantity}, ${parseInt(price) * quantity}</div>`
    ...
    Object.keys(myCart).map(a => displayItem(document.getElementById(a)))
